import Entry from "../models/Entry.js"; // Import Entry model
...
export const getEntryDetailsById = async (req, res) => {
  ...
  try {
    ...
    const entry = await Entry.findOne({ _id: entryId, projectId })
      .populate("projectId"); // Populate project details (optional)

    // If entry not found
    if (!entry) {
      return res.status(404).json({ message: "Entry not found for this project" });
    }
