...
export const getCompletedProjects = async (req, res) => {
  try {
    ...
    entries.forEach((entry) => {
      ...
      // If the project is completed and not already added, store it
      if (project.status === "completed") {
        const projectId = project._id.toString();
        completedProjectsMap[projectId] = project;
      }
    });

    const completedProjects = Object.values(completedProjectsMap);
