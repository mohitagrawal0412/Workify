...
const SubEntryDetails = () => {
    ...
    const [subEntryDetails, setSubEntryDetails] = useState(state?.subentry || null);

    // 🆕 Fetch entry if not passed via state
    useEffect(() => {
        if (!subEntryDetails && entryId) {
            const fetchEntry = async () => {
                try {
                    const res = await axios.get(`/entries/${entryId}`);
                    setSubEntryDetails(res.data);
                } catch (err) {
                    console.error("Error fetching subentry:", err);
                }
            };
            fetchEntry();
        }
    }, [entryId, subEntryDetails]);
    ...
    const handleDelete = () => {
        ...
        navigate("/entryDetails"); // Or wherever you want to go
        closeDeleteModal();
    };
            ...
            <div className="max-w-5xl mx-auto relative bg-white shadow-xl rounded-lg p-8 bg-opacity-80 backdrop-blur-xl">
                ...
                <div className="space-y-8">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap gap-3">
                                                {subEntryDetails.tags.map((tag, idx) => (
                                                    <span key={idx} className="bg-indigo-100 text-indigo-700 px-3 py-1 text-sm rounded-full shadow-sm hover:bg-indigo-200 transition-all duration-300">
                                                        #{tag}
                                                    </span>
                                                ))}
                                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-6">
                                    {subEntryDetails.attachments.map((att, index) => (
                                        <div key={index} className="bg-white rounded-xl p-5 shadow-lg hover:shadow-2xl cursor-pointer transition-all duration-300">
                    ...
                    <div className="mt-8 flex justify-between">
                        <button onClick={openEditModal} className="text-blue-600 hover:text-blue-800 flex items-center text-lg font-medium transition-all duration-300">
                            <FaEdit className="mr-2" />
                            Edit
                        </button>
                        <button onClick={openDeleteModal} className="text-red-600 hover:text-red-800 flex items-center text-lg font-medium transition-all duration-300">
                            <FaTrashAlt className="mr-2" />
                            Delete
                        </button>
                    </div>
            ...
            {modalImage && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition-all duration-300" onClick={closeModal}>
                    <div className="relative bg-white p-8 rounded-lg max-w-3xl">
                        ...
                        <img src={modalImage} alt="Attachment" className="max-w-full max-h-screen object-contain rounded-xl" />
                    </div>
                </div>
            )}

            {/* Add edit/delete modals below this if needed, no changes to those */}
        </div>
    );
};
